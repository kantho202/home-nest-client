...
const AllPropertiesCard = ({ property }) => {
    ...
    return (
        <div
            data-aos="fade-up"
            className="group relative rounded-[5px] overflow-hidden shadow-xl border-1 border-[#dce0e0] p-3 hover:shadow-2xl transition-all duration-500"
        >
            {/* Image Section */}
            <div className="relative h-60 overflow-hidden">
                ...
                <div className="absolute bottom-3 right-3 flex gap-2">
                    <button className="bg-white/80 p-2 rounded-full hover:bg-[#ff3333] hover:text-white transition">
                        <FaHeart />
                    </button>
                </div>
            </div>
            ...
            <div className="p-5 flex flex-col flex-grow">
                <h3 className="text-lg font-bold mb-1">{property_name.slice(0,20)}...</h3>
                ...
                <div className="mt-auto flex justify-end">
                    <Link to={`/properties-details/${_id}`} className="btn btn-sm bg-red-500 text-white">Details</Link>
                </div>
            </div>
        </div>
    );
